...
process_execute (const char *file_name) 
{
  ...
  strlcpy (fn_copy, file_name, PGSIZE);
    ...
    token = strtok_r(copy_file_name, " " , &save);
  ...
  // Sogang OS Pintos Project 2 2020 Fall implemented by HJL
  ...
  for (e = list_begin(children); e != list_end(children); e = list_next(e)) {
       ...
       if (tmp->exit_status == -1) return process_wait(tid);
  }
  //-------------------------------------
...
start_process (void *file_name_)
{
  ...
  // Sogang OS Pintos Project 2 2020 Fall implemented by HJL
  sema_up(&(thread_current()->parent->l_lock));
  //-------------------------------------
...
// ----------------------------
// argument passing implemented by HJL
// Sogang OS Pintos Project 1 2020 Fall
// parse the arguments and save it to the caller's stack
// according to 80x86 calling convention

void make_stack(char *file_name, void **esp) {

  char ** argv;
  int argc, total;
  char copied_file_name[256];
  char *token, *save;
  int i;
  int len;
  
  strlcpy(copied_file_name, file_name, strlen(file_name) + 1);
  token = strtok_r(copied_file_name, " ", &save);
  argc = 0;
